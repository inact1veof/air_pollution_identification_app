"""
Air pollution forecasting class using naive model selection
"""
import warnings
...
class NaiveSelector():
    ...
    """
    def __init__(self, base_models: List, pretrained: bool = False, circles_count: int = 1, start_day: int = 5):
    ...
    def fit(self, df: Union[pd.DataFrame, dsm_timeseries],start_day: int, day_points: int, datetime_column_name: str = None,
        ...
        RMSE_before = np.zeros((self.circles_count * len(self.day_list), self.num_rls_par))
        ...
        for i in range(self.circles_count * len(self.day_list)):
            ...
            for j in range(0, len(copy_from_data.index)):
                ...
                RMSE_before[i, :] = rmse_data

        for m in range(self.circles_count):
            for s in range(0, len(self.day_list)):
                if self.day > start_day:
                    index_min_RMSE = np.where(RMSE_before[self.day - 1, :] == np.amin(RMSE_before[self.day - 1, :]))

                    for t in range(self.day_points):
                        y_estimate[t, self.day] = LOAD_data[index_min_RMSE[0][0], t, self.day]
                        MODEL_number[t, self.day] = index_min_RMSE[0][0] + 1

                self.day = self.day + 1
        self.__y_estimate = y_estimate
        self.__mn = MODEL_number
        ...
        if method == "All":
            ...
            for i in range(len(self.day_list)):
                ...
                for j in range(len(copy_to_data)):
                    ...
                    self.df.loc[l, 'forecast'] = float(self.__y_estimate[j, i + (self.circles_count - 1) * len(self.day_list)])
