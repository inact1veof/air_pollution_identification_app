"""
Air pollution forecasting class using SDays model selection
"""
import warnings
...
class SDaysSelector():
    ...
    :param ndays: using N previous days data for predict (int)
    """
    def __init__(self, base_models: List, pretrained: bool = False, circles_count: int = 1, start_day: int = 5, ndays = 14):
    ...
    def fit(self, df: Union[pd.DataFrame, dsm_timeseries], start_day: int, day_points: int, datetime_column_name: str = None,
        ...
        RMSE_data = np.zeros((4, self.circles_count * len(self.day_list), self.num_rls_par))
        ...
        for i in range(self.circles_count * len(self.day_list)):
            ...
            # for j in range(0, min(self.day_points, len(copy_from_data.index))):
            ...
            rsRMSE = np.zeros((self.num_rls_par))

            if i > self.ndays:
                for k in range(self.ndays):
                    for b in range(self.num_rls_par):
                        rsRMSE[b] = rsRMSE[b] + RMSE_data[0, i - self.ndays + k, b]
            else:
                for b in range(self.num_rls_par):
                    rsRMSE[b] = 0

            RMSE_data[2, i, :] = rsRMSE
            index_min_RMSE = np.where(RMSE_data[2, i, :] == np.amin(RMSE_data[2, i, :]))
            RMSE_data[3, i, index_min_RMSE[0][0]] = 1

        for m in range(self.circles_count):
            for s in range(0, len(self.day_list)):
                if self.day > start_day:
                    BestRMSEs = np.zeros((self.num_rls_par))
                    for k in range(self.num_rls_par):
                        for l in range(self.ndays):
                            BestRMSEs[k] = BestRMSEs[k] + RMSE_data[3, self.day - self.ndays + l, k]

                    index_max_RMSE = np.where(BestRMSEs[:] == np.amax(BestRMSEs[:]))

                    for t in range(self.day_points):
                        y_estimate[t, self.day] = LOAD_data[index_max_RMSE[0][0], t, self.day]
                        MODEL_number[t, self.day] = index_max_RMSE[0][0] + 1
